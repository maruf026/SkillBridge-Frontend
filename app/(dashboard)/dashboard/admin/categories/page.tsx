...
export default function AdminCategoriesPage() {
  ...
  // 1. Fetch Categories using $fetch
  const loadCategories = async () => {
    try {
     
      const { data, error } = await authClient.$fetch<any>("/api/admin/categories");
      ...
      setCategories(data?.data || []); // Accessing the .data property from your backend response
    } catch (err) {
  ...
  const addCategory = async () => {
    ...
    toast.promise(
      async () => {
        const { error } = await authClient.$fetch("/api/admin/categories", {
          method: "POST",
          body: { name }, 
        });

        if (error) {
          if (error.status === 401) throw new Error("Unauthorized: Please re-login");
  ...
  const deleteCategory = async (id: string) => {
    ...
    toast.promise(
      async () => {
        const { error } = await authClient.$fetch(`/api/admin/categories/${id}`, {
          method: "DELETE",
        });
    ...
    <div className="max-w-5xl mx-auto space-y-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-l-4 border-indigo-600 pl-6">
        <div>
          ...
          <p className="text-slate-500 font-bold text-lg mt-1">Organize tutoring subjects</p>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-slate-200 rounded-4xl p-6 shadow-xl shadow-slate-200/50">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            onClick={addCategory}
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-black px-10 py-4 rounded-2xl transition-all"
          >
            Create Category
          </button>
        </div>
